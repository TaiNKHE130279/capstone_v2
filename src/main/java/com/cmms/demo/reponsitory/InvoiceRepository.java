...
public interface InvoiceRepository extends JpaRepository<Invoice,String>
    ...
    @Query(value = "SELECT * FROM `invoices` WHERE (total_amount - advance_amount) > 0 ORDER BY (total_amount - advance_amount) DESC", nativeQuery = true)
    Page<Invoice> getListDebt(Pageable pageable);
}
